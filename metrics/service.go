...
func newMetrics(listenAddress string) (Metrics, error) {
	...
	logger.Notice("New metrics created.")
	return metrics, nil
}
...
func (metrics *metrics) Start() {
	...
	go func() {
		...
		if err != nil {
			logger.WithError(err).Critical("Running metrics HTTP server failed.")
		}
	}()
	logger.Notice("Metrics started.")
}

// Close stops HTTP service and wait until internal goroutine is completed.
// It is the part of implementation of Metrics interface on metrics structure.
func (metrics *metrics) Close() {
	err := metrics.server.Close()
	if err != nil {
		logger.WithError(err).Error("Error closing metrics HTTP server listeners.")
	}
	metrics.serverDone.Wait()
	logger.Notice("Metrics closed.")
}
...
func (metrics *metrics) RegisterGauge(namespace, subsystem, name, help string) (Gauge, error) {
	...
	metrics.registered[combined] = true
