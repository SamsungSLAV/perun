...
var _ = Describe("service", func() {
	...
	Describe("Start/Close", func() {
		...
		It("should log error if server cannot be started", func() {
			...
			log, logerr := testutil.WithStderrMocked(func() {
				...
				err = m.Start()
				Expect(err).NotTo(HaveOccurred())
				// Wait until server is stopped due to error.
				m.(*metrics).serverDone.Wait()
		...
		It("should log error on Close if server's listeners failed to close properly", func() {
			...
			log, logerr := testutil.WithStderrMocked(func() {
				...
				dl.wait()
				err = m.Close()
				Expect(err).To(Equal(errors.New(dummyListenerTestError)))
			})
			Expect(logerr).NotTo(HaveOccurred())
	...
	Describe("RegisterGauge", func() {
		...
		It("should not register same metric twice", func() {
			...
			Expect(logerr).NotTo(HaveOccurred())
			Expect(strings.Count(log, "New gauge registered.")).To(Equal(1))
		})
		It("should fail to register metric not allowed by prometheus", func() {
			...
			Expect(logerr).NotTo(HaveOccurred())
			Expect(log).NotTo(ContainSubstring("New gauge registered."))
		})
	})
})
