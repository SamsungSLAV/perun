...
func (s *HTTPFileServer) Start() error {
	err := filepath.Walk(s.dir, func(path string, info os.FileInfo, e error) error {
		if e != nil {
			return e
		}
		if info.IsDir() {
			fs := http.FileServer(http.Dir(path))
			relative := strings.TrimPrefix(path, s.dir)
			s.serveMux.Handle(relative+"/", http.StripPrefix(relative+"/", fs))
		}
		return nil
	})
	if err != nil {
