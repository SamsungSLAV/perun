...
func (c *crawler) getLinks() (ret []string) {
	...
	return c.parseLinks(resp.Body)
}

// closeBody is a wrapper for body's Close method logging error if appeared.
func (c *crawler) closeBody(body io.Closer) {
	err := body.Close()
	if err != nil {
		logger.WithError(err).WithProperty("url", c.url).
			Warning("Cannot close response body.")
	}
}

// parseLinks acquires links using html tokenizer that reads provided body.
func (c *crawler) parseLinks(body io.Reader) []string {
	return c.parseATags(html.NewTokenizer(body))
}
...
func (c *crawler) getFileInfo() *perun.FileInfo {
	...
	defer c.closeBody(resp.Body)
	return c.parseHeader(resp.Header)
}

// parseHeader parses http header acquiring length and modification date of a file.
func (c *crawler) parseHeader(header http.Header) *perun.FileInfo {
