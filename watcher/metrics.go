...
func (gauges *gauges) registerMetrics(metrics metrics.Metrics) error {
	var err error
	name := func(t TaskType) string { return "current_" + t }
	help := func(t TaskType) string { return "Number of " + t + " running tasks." }
	...
	gauges.workerBusy, err = registerMetric(metrics, workerBusy, "Number of busy workers.")
	if err != nil {
		return err
	}
	gauges.workerIdle, err = registerMetric(metrics, workerIdle, "Number of idle workers.")
	if err != nil {
		return err
	}

	gauges.queueTasks, err = registerMetric(metrics, queuedTasks,
		"Number of tasks pending in queue.")
	if err != nil {
		return err
	}
	gauges.queueSize, err = registerMetric(metrics, queueSize, "Size of the task queue.")
	return err
}
