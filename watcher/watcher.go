...
func (watcher *Watcher) Run(task Task) {
	// queueTasks gauge value is increased before pushing task to queue to avoid situations,
	// when after pushing task to queue, worker consumes the task and decreases gauge before
	// it is increased. Such situations might have caused queueTasks gauge to be negative,
	// which is not allowed by prometheus API.
	watcher.gauges.queueTasks.Inc()
	select {
	case watcher.tasks <- task:
		logger.WithProperty("task", task).Info("New task scheduled.")
	default:
		logger.WithProperty("task", task).Warning("Task dropped. Tasks queue overflow.")
		watcher.gauges.queueTasks.Dec()
	}
}
